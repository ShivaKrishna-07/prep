  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Check if the college is one of our options or "other"
        const isOther = profile.collegeName !== "Vignana Bharathi Institute of technology";
        
        setFormData({
          collegeName: isOther ? "other" : profile.collegeName || "Vignana Bharathi Institute of technology",
          collegeOther: isOther ? profile.collegeName || "" : "",
          year: profile.year || "1st year",
          department: profile.department || "CSE"
        });
        
        setIsOtherCollege(isOther);
      } catch (err) {
          ...
          <div className="w-full md:w-2/3 md:shadow-custom dark:md:border dark:md:border-border rounded-lg flex flex-col justify-between">
            ...
            <div className="flex justify-end gap-4 mt-4 md:mb-4 md:mr-6">
              <button
                type="button"
                onClick={() => router.push('/profile')}
                className="border border-border px-6 py-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition"
              >
                Cancel
              </button>
              <button
